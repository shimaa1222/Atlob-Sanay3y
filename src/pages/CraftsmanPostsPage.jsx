...
const CraftsmanPostsPage = () => {
  ...
  // ✅ جلب المنشورات - بدون فلاتر افتراضية
  const loadPosts = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      console.log('🔵 [CraftsmanPostsPage] Starting to load posts...');
      ...
      if (selectedCraft) params.craft_id = selectedCraft;
      if (selectedCity) params.city = selectedCity;
      if (selectedUrgency) params.urgency = selectedUrgency;
      
      console.log('📤 [CraftsmanPostsPage] Sending params:', params);
      
      const data = await api.getServicePosts(params);
      
      console.log('📥 [CraftsmanPostsPage] Raw API response:', data);
      
      // ✅ استخراج البيانات من عدة صيغ
      let postsArray = [];
      if (data?.posts?.data) {
        postsArray = data.posts.data;
      } else if (data?.data) {
      ...
      console.log(`📊 [CraftsmanPostsPage] Total posts: ${postsWithResponded.length}`);
      setPosts(postsWithResponded);
      
      // ✅ حساب الـ pagination
      const total = data?.posts?.total || data?.total || postsArray.length;
      const perPage = data?.posts?.per_page || data?.per_page || 10;
      const lastPage = Math.ceil(total / perPage) || 1;
      setLastPage(data?.posts?.last_page || data?.last_page || lastPage);
      
    } catch (err) {
      console.error('❌ [CraftsmanPostsPage] Error loading posts:', err);
      setError(err.message || (isArabic ? 'حدث خطأ في جلب المنشورات' : 'Error loading posts'));
      setPosts([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleRespond = async () => {
    ...
    try {
      ...
      await loadPosts(); // ✅ إعادة تحميل البيانات
    } catch (err) {
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 16px' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0 }}>
            {isArabic ? 'طلبات العامة' : '📋 Service Requests'}
          </h1>
        ...
        <div style={{
          ...
          <span>🔄 {isArabic ? 'تحميل' : 'Loading'}: {loading ? '⏳' : '✅'}</span>
          <span>📊 {isArabic ? 'عدد الطلبات' : 'Posts'}: {posts.length}</span>
          <span>📄 {isArabic ? 'صفحة' : 'Page'}: {currentPage}/{lastPage}</span>
          <button
            onClick={() => {
        ...
        {!loading && posts.length === 0 && (
          <div style={{ textAlign: 'center', padding: '80px 20px', background: card, borderRadius: 16, border: `1px solid ${border}` }}>
            ...
            <p style={{ color: sub }}>
              {isArabic ? 'جرب تغيير معايير البحث أو إنشاء طلب جديد' : 'Try changing your search filters or create a new request'}
            </p>
            <button
              onClick={() => {
                setSelectedCraft('');
