...
const CraftsmanSignupPage = () => {
  ...
  // ✅ OTP State (نفس منطق CustomerSignupPage)
  const [otpStep, setOtpStep] = useState('otp'); // 'otp' | 'verify' | 'done'
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ التأكد من تأكيد البريد الإلكتروني أولاً
    if (!verifiedToken) {
    ...
    try {
      ...
      if (formData.longitude) formDataToSend.append('longitude', formData.longitude);
      if (formData.whatsapp) formDataToSend.append('whatsapp', formData.whatsapp);
      // ✅ يُرسل لو السيرفر فعّل تأكيد الإيميل لهذا الإندبوينت (حالياً معطل من جهة الباك إند حسب تعليق api.js)
